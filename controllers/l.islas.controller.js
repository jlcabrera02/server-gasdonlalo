  ...
  try {
    ...
    for (let i = 0; i < islas.length; i++) {
      ...
      if(buscar){
        let restar = i - 1
        let index = restar / 2;
        gasolinas[index].derecha = {id: islas[i].id, direccion: islas[i].direccion, contiene: await consultGas(islas[i].idisla)}
      }else{
        gasolinas.push({numeroIsla: islas[i].nisla, estacion: islas[i].idestacion_servicio, izquierda: {id: islas[i].id, direccion: islas[i].direccion, contiene: await consultGas(islas[i].idisla)}})
      }
    }
...
async function consultGas (id) {
  console.log(id);
  let m = await islaM.findEstacionYTipos(id, "M"), p = await islaM.findEstacionYTipos(id, "P"), d = await islaM.findEstacionYTipos(id, "D");
  return {
    m,p,d
  } 
}
